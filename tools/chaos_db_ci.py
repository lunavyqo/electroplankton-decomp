...
Rebuilds chaos-db.json from COMMITTED data only (no ROM required):
  ...
  near-miss    config/match_attempts.jsonl → best divergences as `div` (like
               sm64ds nearmiss/db.jsonl). Also NONMATCHING srcPath + div=N parse.
...
def load_best_nearmiss(repo: pathlib.Path) -> dict[str, dict]:
    """Best near_miss per functionId from config/match_attempts.jsonl.

    Same role as SM64DS nearmiss/db.jsonl: keep the closest score (lowest
    divergences). Returns functionId → {divergences, srcPath, name, …}.
    """
    ...
    for line in path.read_text(encoding="utf-8", errors="ignore").splitlines():
        ...
        if not fid:
            # rebuild from module+addr when older rows only have those
            mod = r.get("module")
            addr = r.get("addr")
...
def main() -> None:
    ...
    # SM64DS: nearmiss/db.jsonl → div. EP: match_attempts best near_miss → div.
    ...
    print(
        ...
        f"(from {len(nearmiss)} best attempt rows)"
    )
