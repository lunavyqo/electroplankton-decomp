...
def _src_matched_set():
    """Keys for functions that have a non-NONMATCHING file under src/.
    ...
    paths = [cfg / "symbols.txt"]
    if (cfg / "itcm" / "symbols.txt").is_file():
        paths.append(cfg / "itcm" / "symbols.txt")
    if (cfg / "dtcm" / "symbols.txt").is_file():
        paths.append(cfg / "dtcm" / "symbols.txt")
    ov = cfg / "overlays"
    if ov.is_dir():
        paths.extend(sorted(ov.glob("ov*/symbols.txt")))

    for sp in paths:
        if not sp.is_file():
            continue
        # module label
        if sp.parent.name in ("itcm", "dtcm"):
            module = sp.parent.name
        elif sp.parent.parent.name == "overlays":
            module = sp.parent.name
        else:
            module = "arm9"
        for line in sp.read_text(errors="ignore").splitlines():
            ...
            if not m:
                continue
            name, addr_h = m.group(1), m.group(2)
            ...
            if "NONMATCHING" in head:
                continue
            out.add(make_key(module, int(addr_h, 16)))
    return out


def matched_set():
    """Canonical keys of every byte-exact match.

    Union of `progress/matched.jsonl` (local ledger, SM64DS-style) and committed
    `src/` (EP day-to-day source of truth).
    """
    return _key_set(MATCHED) | _src_matched_set()
